...
function integratePlatformParityServerSource(serverSource) {
  ...
  const schemaBlock = [
    ...
    "      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),",
    "      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),",
    "      CONSTRAINT marketplace_skills_name_check CHECK (char_length(name) BETWEEN 1 AND 80),",
    "      CONSTRAINT marketplace_skills_summary_check CHECK (char_length(summary) BETWEEN 1 AND 500),",
    "      CONSTRAINT marketplace_skills_review_check CHECK (review_status IN ('draft','pending_review','approved','rejected','suspended'))",
    "    );",
    "",
    "    CREATE INDEX IF NOT EXISTS marketplace_skills_public_idx",
    "      ON marketplace_skills(published, review_status, updated_at DESC);",
    "    CREATE INDEX IF NOT EXISTS marketplace_skills_creator_idx",
    "      ON marketplace_skills(creator_user_id, updated_at DESC);",
    "",
    ...
    "      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),",
    "      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),",
    "      UNIQUE (user_id, skill_id, scope_key)",
    "    );",
    "",
    "    CREATE INDEX IF NOT EXISTS marketplace_skill_installs_user_idx",
    "      ON marketplace_skill_installs(user_id, enabled, updated_at DESC);"
  ].join("\n");
