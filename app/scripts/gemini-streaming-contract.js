...
async function main() {
  ...
  try {
    ...
    assert.strictEqual(
      google.buildGeminiRequestBody("", [{ role: "user", content: "fast" }], "none").generationConfig,
      undefined
    );
    assert.strictEqual(
    ...
    console.log("PASS Gemini streaming parity: native SSE chat streaming, bounded parsing, 3.8 reasoning controls, thought filtering, and redirect hardening.");
  } finally {
